...
var PARSERS = {
    ...
    'application/json': function(str, callback) {
        str = str.trim();
...
REST.prototype = {
    ...
    getParsedBody: function(callback, limit) {
        var self = this;
        if (!self._parsedBody) {
            self._parsedBody = new DataHolder({
                loader: function(callback) {
                    self.getBody(function(err, body) {
                        ...
                        if (!parser) {
                            callback(new Error('Unrecognized content type for parsing: ' + contentType));
                        }
